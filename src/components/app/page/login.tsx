...
export function LoginForm() {
  ...
  // --- TEMPORARY STATIC LOGIN STATE & HANDLER ---
  ...
  const handleStaticLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (email === 'sofa.haeta@gmail.com' && password === 'hankrazord') {
      // Replace this with actual router push when dynamic login is implemented
      window.location.href = '/en/overview';
    } else {
      setError('Invalid email or password');
    }
  };
  // --- END TEMPORARY STATIC LOGIN ---
  ...
  return (
    <div className="bg-neutral-primary border border-border-default rounded-base shadow-lg p-8 w-full flex flex-col">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleStaticLogin}>
        {error && (
