...
const HeaderMetaActions = ({ t, count, selectedCount, router, handleDeleteSelected }: any) => (
...
const HeaderSearch = ({ t, searchQuery, searchColumns, table }: any) => (
...
const HeaderColumnSearchSelect = ({ table, t, searchQuery, searchColumns }: any) => (
  <DropdownMenu>
    <DropdownMenuTrigger
      render={
    ...
    <DropdownMenuContent align="end" className="w-[160px] p-2 rounded-[12px] bg-neutral-primary border-border-default shadow-lg">
      {table
        .getAllColumns()
        .filter((column: any) => column.getIsVisible() && column.id !== "select" && column.id !== "actions")
        .map((column: any) => (
          <DropdownMenuCheckboxItem
            key={column.id}
            ...
            onCheckedChange={(value) => {
              const newCols = value
                ...
                : searchColumns.filter((c: string) => c !== column.id);
...
const HeaderColumnVisibility = ({ table, t }: any) => (
  <DropdownMenu>
    <DropdownMenuTrigger
      render={
        <Button variant="outline" className="h-9 px-3 border-border-default text-body font-medium hover:bg-neutral-secondary">
      ...
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4 p-4 border-b border-border-default">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full xl:w-auto">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto grid grid-cols-2 sm:flex">
            <HeaderColumnSearchSelect 
              table={table} 
              t={t} 
              searchQuery={searchQuery} 
              searchColumns={searchColumns} 
            />
            <HeaderColumnVisibility 
              table={table} 
              t={t} 
            />
          </div>
